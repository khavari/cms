                        ...
                        <table class="table table-hover table-striped">
                            ...
                            @foreach($categories as $category)
                                <tr class="{{ (isUpdated($category->updated_at))?'updated':'' }}">
                                    <td>
                                        @include('admin.partials.active',['action'=>route('admin.categories.edit', ['id' => $category->id]),'active'=>$category->active])
                                    </td>
                                    <td>{{$category->id}}</td>
                                    <td>{{ str_limit($category->title,32) }}</td>
                                    <td>{{ str_limit($category->slug,60) }}</td>
                                    <td>{{$category->vocabulary->title}}</td>
                                    <td>{{$category->contents()->count()}}</td>
                                    <td>
                                        @include('admin.partials.delete',['action'=>route('admin.categories.destroy', ['id' => $category->id])])
                                        @include('admin.partials.edit',['action'=>route('admin.categories.edit', ['id' => $category->id])])
                                        @include('admin.partials.show',['action'=>route('admin.categories.destroy', ['id' => $category->id])])
